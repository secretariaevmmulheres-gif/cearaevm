...
function PendenciaBadge({ texto, resolvido, onToggle }: { texto: string; resolvido: boolean; onToggle: () => void }) {
  ...
  return (
    <button onClick={onToggle} title={resolvido ? 'Desmarcar como resolvido' : 'Marcar como resolvido'}
      className={cn(
...
function LinhaTabela({ item, delay, isResolvido, onTogglePendencia, itemResolvido }: {
  item: PendenciaMunicipio; delay: number;
  isResolvido: (pendencia: string) => boolean;
  onTogglePendencia: (pendencia: string) => void;
  ...
  return (
    <motion.tr initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }}
      ...
      className={cn('border-b border-border/40 last:border-0 transition-all',
        itemResolvido ? 'opacity-40 bg-emerald-500/3' : 'hover:bg-muted/30')}>
      <td className="py-3 pl-4 pr-3">
        <div className="flex items-center gap-2">
          <div className={cn('w-1.5 h-1.5 rounded-full shrink-0', origemCor.dot)} />
          <span className={cn('text-sm font-semibold', itemResolvido && 'line-through text-muted-foreground')}>
            {item.municipio}
          </span>
        </div>
      </td>
                  ...
                  {([
                    ...
                    <th key={col.field} onClick={() => handleSort(col.field)}
                      ...
                      <span className="flex items-center">
                        {col.label}<SortIcon field={col.field} sortField={sortField} sortDir={sortDir} />
                      </span>
                    </th>
                  ))}
                </tr>
