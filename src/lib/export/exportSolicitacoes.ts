...
export async function exportSolicitacoesToPDF(solicitacoes: Solicitacao[], filterRegiao?: string) {
  const doc = new jsPDF('landscape');

  // ── Capa ──────────────────────────────────────────────────────────────────
  const emAndamento  = solicitacoes.filter(s => ['Recebida','Em análise','Aprovada','Em implantação'].includes(s.status)).length;
  const inauguradas  = solicitacoes.filter(s => s.status === 'Inaugurada').length;
  const comPatrulha  = solicitacoes.filter(s => s.recebeu_patrulha).length;

  await addPdfCover(doc, {
    titulo:    'SOLICITAÇÕES DE IMPLANTAÇÃO',
    subtitulo: 'Acompanhamento de equipamentos solicitados — Ceará',
    colorKey:  'solicitacoes',
    landscape: true,
    ...
    stats: [
      ...
      { label: 'Inauguradas',           valor: inauguradas },
